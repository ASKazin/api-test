    ...
    $.each(data.records, function (key, val) {
        ...
        read_products_html += `<tr>
            ...
            <td>
                ...
                <button class='btn btn-primary btn-sm m-r-10px read-one-product-button' data-id='` + val.id + `'>
                    <span class='glyphicon glyphicon-eye-open'></span> Просмотр
                </button>

                <!-- кнопка для изменения товара -->
                <button class='btn btn-info btn-sm m-r-10px update-product-button' data-id='` + val.id + `'>
                    <span class='glyphicon glyphicon-edit'></span> Редактировать
                </button>

                <!-- кнопка для удаления товара -->
                <button class='btn btn-danger btn-sm delete-product-button' data-id='` + val.id + `'>
